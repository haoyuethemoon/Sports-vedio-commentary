...
async def human(request):
    try:
        ...
        if params['type']=='echo':
            #nerfreals[sessionid].put_msg_txt(params['text'])#修改数字人说出的话
...
async def generate(request):
    try:
        params = await request.json()
        sessionid = params.get('sessionid', 0)
        asyncio.get_event_loop().run_in_executor(None, vllm, params['file'], params['text'])
        nerfreals[sessionid].Pause()
...
async def restart(request):
    try:
        params = await request.json()
        sessionid = params.get('sessionid', 0)
        nerfreals[sessionid].Start()
        nerfreals[sessionid].Restart()
...
if __name__ == '__main__':
    ...
    appasync.router.add_post("/generate", generate)
    appasync.router.add_post("/pause", pause)
    appasync.router.add_post("/restart", restart)
